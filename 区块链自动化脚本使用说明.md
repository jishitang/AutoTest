...
IP1,USERNAME1,PASSWD1|IP2,USERNAME2,PASSWD2
...
该脚本主要做build chain操作，会新建目录~/autotest/FISCO-BCOS/tools/nodes。支持rocksdb，mysql，external三种方式
...
param 7：db类型（rocksdb，mysql，external），可选，默认为rocksdb
...
param 6：可为空，如果为空则只对当前IP进行操作。IP列表  格式为：  IP1,USERNAME1,PASSWD1|IP2,USERNAME2,PASSWD2
...
5. 第四个字段（断言），需要和模式1配合使用，对于模式二不起作用，传空就可以。
